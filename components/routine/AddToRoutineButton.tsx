...
export function AddToRoutineButton({
  ...
  if (done) {
    return (
      <span
        className={`inline-flex items-center justify-center gap-1.5 rounded-2xl bg-emerald-50 text-emerald-700 text-[13px] font-medium px-3 sm:px-3.5 py-1.5 ring-1 ring-emerald-100 ${className}`}
      >
  ...
  return (
    <button
      ...
      onClick={add}
      disabled={pending}
      // Style OUTLINE (maquette user, juil 2026) : fond blanc + bordure et
      // texte verts — contraste avec le CTA promesse plein à sa gauche.
      // min-w-0 + truncate on the label so the button can shrink in flex-1
      // slots (analyse header on mobile) without breaking the row.
      className={`inline-flex items-center justify-center gap-1.5 rounded-2xl bg-white text-emerald-600 ring-1 ring-emerald-500/70 text-xs sm:text-[13px] font-semibold px-3 sm:px-3.5 py-2.5 transition hover:bg-emerald-50 disabled:opacity-60 ${className}`}
    >
