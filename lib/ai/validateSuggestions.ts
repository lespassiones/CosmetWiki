...
const SYSTEM = `Tu vérifies qu'une alternative cosmétique proposée est un REMPLAÇANT LOGIQUE du produit d'origine, c'est-à-dire le MÊME type d'article.
...
async function mistralValidate(items: ValidateItem[]): Promise<ValidateResult[] | null> {
...
export async function validateSuggestions(
  ...
  try {
    const result = await callWithFallback<ValidateResult[] | null>({
      ...
      fallback: async () => ({
        value: await mistralValidate(items),
        provider: "mistral",
      }),
    });
